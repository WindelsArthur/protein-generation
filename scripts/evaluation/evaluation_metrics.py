...
def get_sequence_embeddings(
    ...
    for seq in iterator:
        ...
        with torch.no_grad():
            ...
            # Extract embeddings for actual sequence (excluding special tokens)
...
def calculate_perplexity(
    ...
    # Create batch with all positions masked
    batch_size = seq_len
...
def get_single_sequence_embedding(
    ...
    with torch.no_grad():
        ...
        # Return per-residue representations (excluding special tokens)
...
def calculate_min_soft_alignment_distance(
    ...
    # Extract and normalize mean-pooled embeddings for training sequences
    ...
    # Create GPU FAISS index for fast similarity search
...
def calculate_sliced_wasserstein(
    ...
    for _ in range(n_projections):
        ...
        proj_gen = gen_emb_scaled.dot(direction)
        proj_train = train_emb_scaled.dot(direction)
...
def calculate_toppr_score(
    ...
    return (
        toppr_result["fidelity"],
        toppr_result["diversity"],
        toppr_result["Top_F1"]
    )
...
def calculate_kde_kl_divergence(
    ...
    # Dimensionality reduction with PCA for KDE stability
